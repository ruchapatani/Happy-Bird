...
//..OBJECTS	
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	image = SOIL_load_image( "res/objects/brick1.png", &width, &height, 0, SOIL_LOAD_RGBA );
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image );
	SOIL_free_image_data( image );
...
void draw()
{
	...
	string str22=">CMR Institute of technology > CG lab Mini-Project : Happy Bird ";
string str23=">Sayan Pal - 1CR17CS132";
string str24=">Patani Rucha Sharadkumar - 1CR17CS089";
	string str2=">HAPPY BIRD >Movement: SPACEBAR >Exit: ESC ";
	if(startFlag==0)
	{
	const char * t2 = str2.c_str();
	renderStrokeFont(100,resY-125,0,t2);
	const char * cc = str22.c_str();
	renderStrokeFont(100,resY-50,0,cc);
	const char * cd = str23.c_str();
	renderStrokeFont(100,resY-75,0,cd);
	const char * ce = str24.c_str();
	renderStrokeFont(100,resY-100,0,ce);
	}
